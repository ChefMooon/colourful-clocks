...
public class AlarmClockShapedRecipe implements CraftingRecipe {
    ...
    public AlarmClockShapedRecipe(String group, CraftingBookCategory category, ShapedRecipePattern pattern, ItemStack result, boolean showNotification) {
        this.group = group;
    ...
    public AlarmClockShapedRecipe(String group, CraftingBookCategory category, ShapedRecipePattern pattern, ItemStack result) {
        this(group, category, pattern, result, true);
    }

    public RecipeSerializer<?> getSerializer() {
        return Objects.requireNonNull(BuiltInRegistries.RECIPE_SERIALIZER.get(ColourfulClocksRecipeSerializers.ALARM_CLOCK));
    }
            ...
            if (stack.is(ColourfulClocksTags.CLOCK_TOP_GLASS)) {
                for (BornholmTopGlassTypes glassType : BornholmTopGlassTypes.values()) {
                    if (glassType.getItem().equals(stack.getItem())) {
                        result.set(ColourfulClocksDataComponentTypes.getAlarmClockData(), new AlarmClockComponent(Optional.empty(), Optional.empty(), Optional.of(glassType), Optional.empty(), Optional.of(false)));
                    }
                }
            }
        }
    ...
    public static class Serializer implements RecipeSerializer<AlarmClockShapedRecipe> {
        public static final MapCodec<AlarmClockShapedRecipe> CODEC = RecordCodecBuilder.mapCodec((instance) -> instance.group(
        ...
        ).apply(instance, AlarmClockShapedRecipe::new));
        public static final StreamCodec<RegistryFriendlyByteBuf, AlarmClockShapedRecipe> STREAM_CODEC = StreamCodec.of(AlarmClockShapedRecipe.Serializer::toNetwork, AlarmClockShapedRecipe.Serializer::fromNetwork);

        public MapCodec<AlarmClockShapedRecipe> codec() {
            return CODEC;
        }

        public StreamCodec<RegistryFriendlyByteBuf, AlarmClockShapedRecipe> streamCodec() {
            return STREAM_CODEC;
        }

        private static AlarmClockShapedRecipe fromNetwork(RegistryFriendlyByteBuf buffer) {
            ...
            return new AlarmClockShapedRecipe(string, craftingBookCategory, shapedRecipePattern, itemStack, bl);
        }

        private static void toNetwork(RegistryFriendlyByteBuf buffer, AlarmClockShapedRecipe recipe) {

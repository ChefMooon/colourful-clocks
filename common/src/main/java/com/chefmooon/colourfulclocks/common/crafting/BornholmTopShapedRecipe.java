...
public class BornholmTopShapedRecipe implements CraftingRecipe {
    ...
    public BornholmTopShapedRecipe(String group, CraftingBookCategory category, ShapedRecipePattern pattern, ItemStack result, boolean showNotification) {
        this.group = group;
    ...
    public BornholmTopShapedRecipe(String group, CraftingBookCategory category, ShapedRecipePattern pattern, ItemStack result) {
        this(group, category, pattern, result, true);
    }

    public RecipeSerializer<?> getSerializer() {
        return Objects.requireNonNull(BuiltInRegistries.RECIPE_SERIALIZER.get(ColourfulClocksRecipeSerializers.BORNHOLM_TOP));
    }
            ...
            if (stack.is(ColourfulClocksTags.CLOCK_TOP_GLASS)) {
                for (BornholmTopGlassTypes glassType : BornholmTopGlassTypes.values()) {
                    if (glassType.getItem().equals(stack.getItem())) {
                        result.set(ColourfulClocksDataComponentTypes.getBornholmTopGlassData(), new BornholmTopGlassComponent(glassType, Optional.empty(), false));
                    }
                }
            }
        }
    ...
    public static class Serializer implements RecipeSerializer<BornholmTopShapedRecipe> {
        public static final MapCodec<BornholmTopShapedRecipe> CODEC = RecordCodecBuilder.mapCodec((instance) -> instance.group(
        ...
        ).apply(instance, BornholmTopShapedRecipe::new));
        public static final StreamCodec<RegistryFriendlyByteBuf, BornholmTopShapedRecipe> STREAM_CODEC = StreamCodec.of(BornholmTopShapedRecipe.Serializer::toNetwork, BornholmTopShapedRecipe.Serializer::fromNetwork);

        public MapCodec<BornholmTopShapedRecipe> codec() {
            return CODEC;
        }

        public StreamCodec<RegistryFriendlyByteBuf, BornholmTopShapedRecipe> streamCodec() {
            return STREAM_CODEC;
        }

        private static BornholmTopShapedRecipe fromNetwork(RegistryFriendlyByteBuf buffer) {
            ...
            return new BornholmTopShapedRecipe(string, craftingBookCategory, shapedRecipePattern, itemStack, bl);
        }

        private static void toNetwork(RegistryFriendlyByteBuf buffer, BornholmTopShapedRecipe recipe) {

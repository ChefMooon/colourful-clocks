...
public class HandbellBlockEntityRendererImpl<T extends HandbellBlockEntity> extends HandbellBlockEntityRenderer implements BlockEntityRenderer<T> {
    ...
    public void render(T blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        ResourceLocation handbellLocation = TextUtil.res(ColourfulClocksModels.HANDBELL_PATH.formatted(level.getBlockState(blockEntity.getBlockPos()).getBlock().getDescriptionId().replace("block.colourfulclocks.", "").replace("_handbell", "")));
